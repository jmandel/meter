...
- Wrap FHIR resource names, datatypes, profiles, elements, and similar identifiers in backticks, e.g. \`Patient\`, \`CapabilityStatement\`, \`Observation.status\`.
...
- When a motion is present, format it compactly and preserve mover, seconder, and vote counts exactly when known.
...
- Wrap FHIR resource names, datatypes, profiles, elements, and similar identifiers in backticks, e.g. \`Patient\`, \`CapabilityStatement\`, \`Observation.status\`.
- If a follow-up has an owner, keep it inline as \`TODO(Name): ...\` rather than converting it into prose.
...
- Normalize FHIR tracker references to canonical HL7 Jira links like [FHIR-34735](https://jira.hl7.org/browse/FHIR-34735).
- Wrap FHIR resource names, datatypes, profiles, elements, and similar identifiers in backticks, e.g. \`Patient\`, \`CapabilityStatement\`, \`Observation.status\`.
- When a formal motion or vote is stated, record the mover, seconder, and vote counts exactly when they are known.
- If a decision concerns a specific tracker, keep the tracker ID in the same bullet as the decision rather than separating the rationale from the issue reference.
...
- If an action item or formal outcome appears, break out of narrative mode long enough to state it explicitly rather than burying it in prose.
